...
cover:
  ...
  type: "image/png"
  alt: "Traveler checking mobile internet beside a ferry pier in Istanbul"
published: true
---
...
For a conventional one- or two-week holiday, this normally does not mean that a visitor must rush to register the phone: the trip ends well before the stated period. The rule matters more to long-stay visitors, residents, and people who continue using the same imported device in Turkey. Do not assume that switching from a physical SIM to an eSIM resets or conceals the device identity. Mobile networks still see the phone’s IMEI.
...
- **Home roaming** involves the least setup, but you need to confirm the destination, allowance, and price in your current plan.
...
3. Buy a plan for Turkey or a regional plan that explicitly lists Turkey in its coverage.
4. Install the eSIM at home over stable Wi‑Fi and label the line “Turkey data” or another clear name.
5. Save the QR code, order details, and setup instructions offline. Do not delete an installed profile without provider guidance: a single-use QR code may not support reinstallation.
6. Download an offline map for the airport and accommodation area, plus the hotel address in Turkish.
7. Update transport, banking, translation, and messaging apps before the flight.

Unless the provider instructs otherwise, you do not need to use the travel line for cellular data at home. The pre-departure goal is to install it successfully and make sure it appears in the phone’s SIM settings.

## Settings to check after landing

After leaving airplane mode, enable the travel eSIM, select it for cellular data, and turn on data roaming for that line. A travel eSIM connects through a partner network, so roaming on the travel line is commonly required. At the same time, verify that cellular data is disabled for the home SIM and that automatic data switching cannot move traffic back to it.

If the connection does not appear immediately, wait a few minutes and check:

- the correct eSIM is enabled;
- it is selected as the cellular-data line;
- data roaming is on for the travel line;
- network selection is set to automatic;
- the package has not expired or run out of data;
- the order instructions specify a particular APN.

Do not copy an APN from an unrelated forum post. Change it only when the instructions for your specific plan require it. The carrier name shown on the phone may also differ from Yotti because the device can display a local or technical network partner.

## Five Turkey-specific moments when data matters

Mobile internet is useful well beyond social media. In Turkey, it can be important immediately after arrival:

- showing a driver the hotel address in Turkish;
- contacting a pre-booked transfer outside the terminal;
- navigating between an airport and the city;
- checking a ferry, coach, or domestic-flight update;
- opening translation and banking tools away from hotel Wi‑Fi.

Keep critical addresses and tickets offline because even a correctly installed eSIM may take a few minutes to register. Treat public Wi‑Fi as a backup rather than the foundation of the journey. Avoid entering payment credentials on an open network and disable automatic joining for unknown hotspots.
...
For a typical visit, the resilient setup is an unlocked compatible phone, an eSIM installed before departure, the travel line selected for data, unintended home-SIM roaming disabled, and essential information saved offline. Be aware of the 120-day rule without confusing it with mandatory registration for every short holiday. It becomes a critical planning issue when an imported phone will be used in Turkey for an extended period. Verify regulatory details with BTK, plan conditions on the live provider page, and never delete a working eSIM merely as a first troubleshooting step.
