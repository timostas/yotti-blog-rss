...
Перед изменением очереди или подготовкой нового выпуска автоматический контур
отдельно проверяет структурное состояние публичного импортёра Yotti:
...
Решение владельца от 14 сентября: ранее подтверждённые ограничения импортёра
не исследовать и не обсуждать повторно, не задерживать из-за них редактуру.
Контроль фактов, качества исходников, ссылок и переноса контента сохраняется.
Отдельные performance-замеры — по запросу или новой наблюдаемой регрессии;
прежние FAIL не становятся PASS. Нормативное правило:
`production.knownImporterDebtPolicy` в `config/editorial-policy.json`.

С 13 сентября 2026 года редакция использует Astra/high, для сложных маршрутов
Astra/xhigh. Критерии и журнал проверок:
`docs/astra-editorial-acceptance.md`. Для восстановления пропусков действует
тот же стандарт без снижения требований ради количества.
...
Когда требуется отдельный performance-аудит после ручной синхронизации,
публичный capture сохраняется только в локальной непубличной директории
и проверяется так:
